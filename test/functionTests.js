...
describe("Governance Function Tests", function () {
  ...
  it("Test Constructor()", async function () {
    assert(
      (await gov.tellor()) == flex.address,
      "tellor address should be correct"
    );
    assert(
      (await gov.oracle()) == flex.address,
      "oracle address should be set"
    );
    assert((await gov.token()) == token.address, "token address should be set");
    assert(
      (await gov.getDisputeFee()) == (await flex.getStakeAmount()) / 10,
      "min dispute fee should be set properly"
    );
    assert(
      (await gov.teamMultisig()) == accounts[0].address,
      "team multisig should be set correctly"
    );
  });
  it("Test beginDispute()", async function () {
    ...
    ); //no value exists for the timestamp provided
    await flex
      .connect(accounts[1])
    ...
    ); // must have tokens to pay/begin dispute
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("10"));
    ...
    assert(
      balance1 - balance2 - (await flex.getStakeAmount()) / 10 == 0,
  ...
  it("Test executeVote()", async function () {
    ...
    assert(
      balance1 - balance2 - web3.utils.toWei("1") == 0,
      "dispute fee paid should be correct"
    );
  ...
  it("Test tallyVotes()", async function () {
    ...
    await token
      .connect(accounts[1])
      .transfer(accounts[2].address, web3.utils.toWei("20"));
    await token
      .connect(accounts[2])
      .approve(flex.address, web3.utils.toWei("20"));
    await flex.connect(accounts[2]).depositStake(web3.utils.toWei("20"));
    await flex
      .connect(accounts[2])
      .submitValue(ETH_QUERY_ID, h.uintTob32(100), 0, ETH_QUERY_DATA);
    blocky = await h.getBlock();
    ...
    assert(
      (await token.balanceOf(accounts[2].address)) == 0,
      "should not have tokens returned"
    );
  });
  it("Test vote()", async function () {
    ...
    voteInfo = await gov.getVoteInfo(1);
    assert(
      voteInfo[1][5] - (await token.balanceOf(accounts[1].address)) == 0,
      "Tokenholders doesSupport tally should be correct"
    );
    assert(
      voteInfo[1][6] == web3.utils.toWei("10"),
      "Tokenholders against tally should be correct"
    );
    ...
    assert(
      voteInfo[1][11] == 0,
      "Reporters doesSupport tally should be correct"
    );
    ...
    assert(
      voteInfo[1][14] == 0,
      "teamMultisig doesSupport tally should be correct"
    );
    assert(
      voteInfo[1][15] == 0,
      "teamMultisig against tally should be correct"
    );
    assert(
      voteInfo[1][16] == 0,
      "teamMultisig invalid tally should be correct"
    );
    assert(
      await gov.didVote(1, accounts[1].address),
      "Voter's voted status should be correct"
    );
    assert(
      await gov.didVote(1, accounts[2].address),
      "Voter's voted status should be correct"
    );
    assert(
      (await gov.didVote(1, accounts[3].address)) == false,
      "Voter's voted status should be correct"
    );
    assert(
      (await gov.getVoteTallyByAddress(accounts[1].address)) == 1,
      "Vote tally by address should be correct"
    );
    assert(
      (await gov.getVoteTallyByAddress(accounts[2].address)) == 1,
      "Vote tally by address should be correct"
    );
  });
  it("Test voteOnMultipleDisputes()", async function () {
    ...
    assert(
      voteInfo1[1][5] == h.toWei("100"),
      "Dispute1 Tokenholders doesSupport tally should be correct"
    );
    assert(
      voteInfo2[1][6] == h.toWei("100"),
      "Dispute2 Tokenholders against tally should be correct"
    );
    assert(
      voteInfo3[1][7] == h.toWei("100"),
      "Dispute3 Tokenholders invalid tally should be correct"
    );
  });
  it("Test didVote()", async function () {
    ...
    await gov.connect(accounts[1]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await gov.didVote(1, accounts[1].address)) == false,
      "Voter's voted status should be correct"
    );
    await gov.connect(accounts[1]).vote(1, true, false);
    assert(
  ...
  it("Test getDisputeInfo()", async function () {
    ...
    assert(
      disputeInfo[0] == ETH_QUERY_ID,
      "Disputed query id should be correct"
    );
    assert(
      disputeInfo[1] == blocky.timestamp,
      "Disputed timestamp should be correct"
    );
    assert(
      disputeInfo[2] == h.uintTob32(100),
      "Disputed value should be correct"
    );
    assert(
      disputeInfo[3] == accounts[2].address,
      "Disputed reporter should be correct"
    );
  });
  it("Test getOpenDisputesOnId()", async function () {
    ...
    await token
      .connect(accounts[1])
      .transfer(accounts[3].address, web3.utils.toWei("20"));
    await token
      .connect(accounts[3])
      .approve(flex.address, web3.utils.toWei("20"));
    ...
    await token
      ...
      .approve(gov.address, web3.utils.toWei("10"));
    assert(
      (await gov.getOpenDisputesOnId(ETH_QUERY_ID)) == 0,
      "Open disputes on ID should be correct"
    );
    await gov.connect(accounts[1]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
    ...
    assert(
      (await gov.getOpenDisputesOnId(ETH_QUERY_ID)) == 2,
      "Open disputes on ID should be correct"
    );
    await gov.connect(accounts[1]).vote(1, true, false);
    await h.advanceTime(86400 * 2);
  ...
  it("Test getVoteCount()", async function () {
    ...
    await gov.connect(accounts[1]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await gov.getVoteCount()) == 1,
      "Vote count should increment correctly"
    );
    await h.advanceTime(86400 * 2);
    await gov.connect(accounts[1]).tallyVotes(1);
    await h.advanceTime(86400);
  ...
  it("Test getVoteInfo()", async function () {
    ...
    await gov
      .connect(accounts[1])
      .beginDispute(ETH_QUERY_ID, blocky0.timestamp);
    blocky1 = await h.getBlock();
    await gov.connect(accounts[1]).vote(1, true, false);
    ...
    hash = ethers.utils.solidityKeccak256(
      ["bytes32", "uint256"],
      [ETH_QUERY_ID, blocky0.timestamp]
    );
    assert(voteInfo[0] == hash, "Vote hash should be correct");
    assert(voteInfo[1][0] == 1, "Vote round should be correct");
    assert(
      voteInfo[1][1] == blocky1.timestamp,
      "Vote start date should be correct"
    );
    assert(
      voteInfo[1][2] == blocky1.number,
      "Vote blocknumber should be correct"
    );
    assert(
      voteInfo[1][3] - web3.utils.toWei("1") == 0,
      "Vote fee should be correct"
    );
    assert(
      voteInfo[1][4] == blocky2.timestamp,
      "Vote tallyDate should be correct"
    );
    assert(
      voteInfo[1][5] == web3.utils.toWei("979"),
      "Vote tokenholders doesSupport should be correct"
    );
    ...
    assert(
      voteInfo[1][11] == 0,
      "Vote reporters doesSupport should be correct"
    );
    ...
    assert(
      voteInfo[1][14] == 0,
      "Vote teamMultisig doesSupport should be correct"
    );
    ...
    assert(
      voteInfo[4] == accounts[1].address,
      "Vote initiator address should be correct"
    );
  });
  ...
  it("Test getVoteTallyByAddress()", async function () {
    ...
    await gov
      ...
      .beginDispute(ETH_QUERY_ID, blocky0.timestamp);
    assert(
      (await gov.getVoteTallyByAddress(accounts[1].address)) == 0,
    ...
    assert(
      (await gov.getVoteTallyByAddress(accounts[1].address)) == 2,
      "Vote tally should be correct"
    );
  });
  it("Test _getUserTips", async function () {
    ...
    await token
      .connect(userAccount)
      .approve(autopay.address, web3.utils.toWei("20"));
    await autopay
      .connect(userAccount)
      .tip(ETH_QUERY_ID, web3.utils.toWei("20"), h.bytes(100));
    await flex
      .connect(accounts[2])
      .submitValue(ETH_QUERY_ID, h.uintTob32(100), 0, ETH_QUERY_DATA);
    blocky = await h.getBlock();
    ...
    assert(
      voteInfo[1][8] == web3.utils.toWei("20"),
      "Vote users doesSupport weight should be based on tip total"
    );
  });
});
