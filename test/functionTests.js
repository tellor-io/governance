...
describe("Governance Function Tests", function() {
  ...
  it("Test beginDispute()", async function() {
    ...
    voteCount = await gov.getVoteCount()
    assert(voteCount._voteCount == 1, "vote count should be correct")
    assert(voteCount._openVoteCount == 1, "open vote count should be correct")
  ...
  it("Test executeVote()", async function() {
    ...
    voteCount = await gov.getVoteCount()
    assert(voteCount._voteCount == 1, "vote count should be correct")
    assert(voteCount._openVoteCount == 0, "open vote count should be correct")
  ...
  it("Test getVoteCount()", async function() {
    voteCount = await gov.getVoteCount()
    assert(voteCount._voteCount == 0, "Vote count should start at 0")
    assert(voteCount._openVoteCount == 0, "Open vote count should start at 0")
    await token.connect(accounts[1]).transfer(accounts[2].address, web3.utils.toWei("20"))
    await token.connect(accounts[2]).approve(flex.address, web3.utils.toWei("20"))
    await flex.connect(accounts[2]).depositStake(web3.utils.toWei("20"))
    await flex.connect(accounts[2]).submitValue(h.uintTob32(1), h.uintTob32(100), 0, '0x')
    ...
    await gov.connect(accounts[1]).beginDispute(h.uintTob32(1), blocky.timestamp)
    voteCount = await gov.getVoteCount()
    assert(voteCount._voteCount == 1, "Vote count should increment correctly")
    assert(voteCount._openVoteCount == 1, "Open vote count should increment correctly")
    await h.advanceTime(86400 * 2)
    await gov.connect(accounts[1]).tallyVotes(1)
    await h.advanceTime(86400)
    await gov.executeVote(1)
    voteCount = await gov.getVoteCount()
    assert(voteCount._voteCount == 1, "Vote count should not change after vote execution")
    assert(voteCount._openVoteCount == 0, "Open vote count should decrement correctly")
    await token.connect(accounts[1]).transfer(accounts[2].address, web3.utils.toWei("20"))
    await token.connect(accounts[2]).approve(flex.address, web3.utils.toWei("20"))
    await flex.connect(accounts[2]).depositStake(web3.utils.toWei("20"))
    await flex.connect(accounts[2]).submitValue(h.uintTob32(1), h.uintTob32(100), 0, '0x')
    ...
    await gov.connect(accounts[1]).beginDispute(h.uintTob32(1), blocky.timestamp)
    voteCount = await gov.getVoteCount()
    assert(voteCount._voteCount == 2, "Vote count should increment correctly")
    assert(voteCount._openVoteCount == 1, "Open vote count should increment correctly")
  });
