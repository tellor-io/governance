...
describe("Governance End-To-End Tests", function () {
  ...
  it("Test multiple disputes", async function () {
    await token
      .connect(accounts[1])
      .approve(flex.address, web3.utils.toWei("1000"));
    await flex.connect(accounts[1]).depositStake(web3.utils.toWei("100"));
    await token
      .connect(accounts[1])
      .transfer(accounts[2].address, web3.utils.toWei("100"));
    await token
      .connect(accounts[1])
      .transfer(accounts[3].address, web3.utils.toWei("100"));
    await token
      .connect(accounts[1])
      .transfer(accounts[4].address, web3.utils.toWei("100"));
    await token
      .connect(accounts[1])
      .transfer(accounts[5].address, web3.utils.toWei("100"));
    await token
      .connect(accounts[4])
      .approve(flex.address, web3.utils.toWei("100"));
    ...
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("10"));
    let balance1 = await token.balanceOf(accounts[2].address);
    await h.expectThrow(
      gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp)
    ); //no value exists
    await flex
      .connect(accounts[1])
      .submitValue(ETH_QUERY_ID, h.bytes(100), 0, ETH_QUERY_DATA);
    blocky = await h.getBlock();
    await flex
      .connect(accounts[4])
      .submitValue(h.hash("0x123456"), h.bytes(200), 0, "0x123456");
    let blocky2 = await h.getBlock();
    await flex
      .connect(accounts[5])
      .submitValue(h.hash("0x1234"), h.bytes("a"), 0, "0x1234");
    let blocky3 = await h.getBlock();
    await h.expectThrow(
      gov.connect(accounts[4]).beginDispute(ETH_QUERY_ID, blocky.timestamp)
    ); // must have tokens to
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("30"));
    await gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    await gov
      .connect(accounts[2])
      .beginDispute(h.hash("0x123456"), blocky2.timestamp);
    await gov
      .connect(accounts[2])
      .beginDispute(h.hash("0x1234"), blocky3.timestamp);
    ...
    assert(
      (await gov.getOpenDisputesOnId(ETH_QUERY_ID)) == 1,
      "open disputes on ID should be correct"
    );
    assert(
      (await gov.getVoteRounds(_hash)) == 1,
      "number of vote rounds should be correct"
    );
    vars = await gov.getDisputeInfo(2);
    _hash = await ethers.utils.solidityKeccak256(
      ["bytes32", "uint256"],
      [h.hash("0x123456"), blocky2.timestamp]
    );
    ...
    assert(
      (await gov.getOpenDisputesOnId(h.hash("0x123456"))) == 1,
      "open disputes on ID should be correct"
    );
    ...
    _hash = ethers.utils.solidityKeccak256(
      ["bytes32", "uint256"],
      [h.hash("0x1234"), blocky3.timestamp]
    );
    ...
    assert(
      (await gov.getOpenDisputesOnId(h.hash("0x1234"))) == 1,
      "open disputes on ID should be correct"
    );
  ...
  it("Test 1000 rounds of a dispute, each of 3 outcomes", async function () {
    ...
    await token.mint(accounts[2].address, h.toWei("30000")); //disputer
    await token.connect(accounts[1]).approve(flex.address, h.toWei("1000"));
    await token.connect(accounts[2]).approve(gov.address, h.toWei("30000"));

    // outcome: PASSED
    await flex.connect(accounts[1]).depositStake(h.toWei("1000"));
    await flex
      .connect(accounts[1])
      .submitValue(ETH_QUERY_ID, h.bytes(100), 0, ETH_QUERY_DATA);
    blocky = await h.getBlock();
    ...
    for (i = 0; i < 1000; i++) {
      await gov
        .connect(accounts[2])
        .beginDispute(ETH_QUERY_ID, blocky.timestamp);
      await gov.connect(accounts[3]).vote(i + 1, true, false);
      await h.advanceTime(86400 * 6);
      await gov.tallyVotes(i + 1);
    }

    encoded = web3.utils.encodePacked(
      { value: ETH_QUERY_ID, type: "bytes32" },
      { value: blocky.timestamp, type: "uint256" }
    );
    hash = h.hash(encoded);
    voteRounds = await gov.getVoteRounds(hash);
    assert(voteRounds.length == 1000, "Should be 1000 vote rounds");

    await h.advanceTime(86400 * 2);
    await gov.executeVote(1000);
    voteInfo = await gov.getVoteInfo(1000);
    assert(voteInfo[3] == 1, "Vote should be correct");
    ...
    for (i = 0; i < 1000; i++) {
      await gov
        .connect(accounts[2])
        .beginDispute(ETH_QUERY_ID, blocky.timestamp);
      await gov.connect(accounts[3]).vote(1000 + i + 1, false, false);
      await h.advanceTime(86400 * 6);
      await gov.tallyVotes(1000 + i + 1);
    }
    ...
    await h.advanceTime(86400 * 2);
    await gov.executeVote(2000);
    voteInfo = await gov.getVoteInfo(2000);
    assert(voteInfo[3] == 0, "Vote should be correct");
    ...
    for (i = 0; i < 1000; i++) {
      await gov
        .connect(accounts[2])
        .beginDispute(ETH_QUERY_ID, blocky.timestamp);
      await gov.connect(accounts[3]).vote(2000 + i + 1, false, true);
      await h.advanceTime(86400 * 6);
      await gov.tallyVotes(2000 + i + 1);
    }
    ...
    await h.advanceTime(86400 * 2);
    await gov.executeVote(3000);
    voteInfo = await gov.getVoteInfo(3000);
    assert(voteInfo[3] == 2, "Vote should be correct");
  });

  it("Test no votes on a dispute", async function () {
    await token
      .connect(accounts[1])
      .approve(flex.address, web3.utils.toWei("10"));
    ...
    await gov.executeVote(1);
    assert(
      (await token.balanceOf(accounts[1].address)) - balance1 ==
        web3.utils.toWei("10"),
      "account1 balance should increase by original stake amount"
    );
    assert(
      (await token.balanceOf(accounts[2].address)) - balance2 ==
        web3.utils.toWei("1"),
      "account2 balance should increase by fee amount"
    );
  ...
  it("Test multiple vote rounds on a dispute, all passing", async function () {
    ...
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("20"));
    await gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    ...
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("40"));
    await gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    ...
    assert(
      (await token.balanceOf(accounts[1].address)) - balance1 == 0,
      "account1 balance should not change"
    );
    assert(
      (await token.balanceOf(accounts[2].address)) - balance2 ==
        web3.utils.toWei("17"),
      "account2 balance should increase by original stake amount plus fee amount"
    );
    assert(
      balanceGov - (await token.balanceOf(gov.address)) ==
        web3.utils.toWei("17"),
      "governance balance should decrease by original stake amount plus fee amount"
    );
  ...
  it("Test multiple vote rounds on a dispute,  overturn result", async function () {
    ...
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("1"));
    await gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    await gov.connect(accounts[1]).vote(1, true, false);
    await gov.connect(accounts[2]).vote(1, true, false);
    await h.advanceTime(86400 * 2);
    ...
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("2"));
    await gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    await gov.connect(accounts[1]).vote(2, true, false);
    await gov.connect(accounts[2]).vote(2, true, false);
    await h.advanceTime(86400 * 4);
    ...
    await token
      .connect(accounts[2])
      .approve(gov.address, web3.utils.toWei("4"));
    await gov.connect(accounts[2]).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    ...
    assert(
      (await token.balanceOf(accounts[1].address)) - balance1 ==
        web3.utils.toWei("17"),
      "account1 balance should increase by original stake amount plus fee amount"
    );
    assert(
      (await token.balanceOf(accounts[2].address)) - balance2 == 0,
      "account2 balance should not change"
    );
    assert(
      balanceGov - (await token.balanceOf(gov.address)) ==
        web3.utils.toWei("17"),
  ...
  it("Test voting from all four stakeholder groups", async function () {
    ...
    await flex
      .connect(accounts[19])
      .submitValue(autopayQueryId, autopayArray, 0, autopayQueryData);
    await h.advanceTime(86400);
    // define stakeholders
    user1 = accounts[9];
    user2 = accounts[10];
    ...
    await autopay
      .connect(user1)
      .tip(ETH_QUERY_ID, web3.utils.toWei("1"), ETH_QUERY_DATA);
    // // set user2
    ...
    await token
      .connect(accounts[1])
      .transfer(tokenholder2.address, web3.utils.toWei("20"));
    // submit some reporter values
    await token
      .connect(accounts[1])
      .transfer(reporter1.address, web3.utils.toWei("10"));
    await token
      .connect(accounts[1])
      .transfer(reporter2.address, web3.utils.toWei("10"));
    await token
      .connect(reporter1)
      .approve(flex.address, web3.utils.toWei("10"));
    await token
    ...
    await flex
      .connect(reporter2)
      .submitValue(h.hash("0xabcd"), h.bytes(100), 0, "0xabcd");
    // dispute value
    await token
      .connect(accounts[1])
      .approve(gov.address, web3.utils.toWei("10"));
    ...
    voteInfo = await gov.getVoteInfo(1);
    assert(
      voteInfo[1][5] == web3.utils.toWei("959"),
      "Tokenholders doesSupport should be correct"
    );
    assert(
      voteInfo[1][6] == web3.utils.toWei("30"),
      "Tokenholders against should be correct"
    );
    assert(voteInfo[1][7] == 0, "Tokenholders invalid should be correct");
    assert(
      voteInfo[1][8] == web3.utils.toWei("1"),
      "Users doesSupport should be correct"
    );
    assert(
      voteInfo[1][9] == web3.utils.toWei("1"),
      "Users against should be correct"
    );
  ...
  it("Test query id value after a dispute", async () => {
    ...
    await flex
      .connect(accounts[1])
      .submitValue(ETH_QUERY_ID, h.bytes(200), 0, ETH_QUERY_DATA);
    blocky = await h.getBlock();
    assert(
      (await flex.getCurrentValue(ETH_QUERY_ID)) == h.bytes(200),
      "Current value should be most recent report"
    );
    ...
    await flex
      ...
      .submitValue(ETH_QUERY_ID, h.bytes(100), 0, ETH_QUERY_DATA);
    assert(
      (await flex.getCurrentValue(ETH_QUERY_ID)) == h.bytes(100),
      "Current value should be most recent report"
    );
    blocky = await h.getBlock();
    await token
    ...
    assert(
      (await flex.getCurrentValue(ETH_QUERY_ID)) == h.bytes(200),
      "Current value shouldn't be dispute value"
    );
  });
  it("Cannot vote on dispute id 0", async function () {
    ...
    await token
      .connect(accounts[1])
      .transfer(accounts[2].address, h.toWei("100"));
  ...
  it("Test 1000 rounds of a dispute -- different ts", async function () {
    ...
    await token
      .connect(accounts[1])
      .approve(flex.address, h.toWei("10000000000000000000000"));
    await token.connect(accounts[2]).approve(gov.address, h.toWei("10000"));
    await flex
      .connect(accounts[1])
      .depositStake(h.toWei("10000000000000000000000"));
    for (i = 0; i < 1000; i++) {
      let qd = abiCoder.encode(["string", "uint"], ["SpotPrice", i]);
      let qi = ethers.utils.keccak256(qd);
      await flex.connect(accounts[1]).submitValue(qi, h.bytes(100), 0, qd);
      blocky = await h.getBlock();
      await gov.connect(accounts[2]).beginDispute(qi, blocky.timestamp);
    }
    for (i = 0; i < 1000; i++) {
      await h.advanceTime(86400 * 6);
      await gov.tallyVotes(i + 1);
      await h.advanceTime(86400 * 2);
      await gov.executeVote(i + 1);
    }
  });
  ...
  it("Test array of dispute ids by reporter address", async function () {
    await token
      .connect(accounts[1])
      .transfer(accounts[2].address, h.toWei("20"));
    await token
      .connect(accounts[1])
      .transfer(accounts[3].address, h.toWei("50"));
    ...
    await gov
      .connect(accounts[3])
      .beginDispute(ETH_QUERY_ID, blocky1.timestamp);
    ...
    await gov
      .connect(accounts[3])
      .beginDispute(ETH_QUERY_ID, blocky2.timestamp);
    disputeIds = await gov.getDisputesByReporter(accounts[2].address);
    assert(disputeIds.length == 1, "Should be one dispute id");
    ...
    await flex
      .connect(accounts[2])
      .submitValue(ETH_QUERY_ID, h.uintTob32(300), 0, ETH_QUERY_DATA);
    blocky3 = await h.getBlock();
    await gov
      .connect(accounts[3])
      .beginDispute(ETH_QUERY_ID, blocky3.timestamp);
    disputeIds = await gov.getDisputesByReporter(accounts[2].address);
  ...
  it("test vote weight outcomes", async function () {
    ...
    await token
      .connect(accounts[1])
      .transfer(accounts[0].address, h.toWei("1000"));
  ...
  it("Dispute fee capped at stake amount, one report", async function () {
    ...
    blocky = await h.getBlock();
    assert(
      (await flex.getStakeAmount()) == h.toWei("10"),
      "Stake amount should be correct"
    );
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("100"),
      "Disputer should have correct balance"
    );
    ...
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("99"),
      "Disputer should have correct balance"
    );
    voteInfo = await gov.getVoteInfo(1);
    ...
    await gov.connect(disputer).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("97"),
      "Disputer should have correct balance"
    );
    ...
    await gov.connect(disputer).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("93"),
      "Disputer should have correct balance"
    );
    voteInfo = await gov.getVoteInfo(3);
    ...
    await gov.connect(disputer).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("85"),
      "Disputer should have correct balance"
    );
    voteInfo = await gov.getVoteInfo(4);
    ...
    await gov.connect(disputer).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("75"),
      "Disputer should have correct balance"
    );
    ...
    await gov.connect(disputer).beginDispute(ETH_QUERY_ID, blocky.timestamp);
    assert(
      (await token.balanceOf(disputer.address)) == h.toWei("65"),
      "Disputer should have correct balance"
    );
  ...
  it("Test voting 10000 times in 5 different rounds", async function () {
    ...
    await token.mint(
      accounts[1].address,
      h.toWei("10000000000000000000000000")
    );
    await token.mint(accounts[2].address, h.toWei("10000"));
    await token
      .connect(accounts[1])
      .approve(flex.address, h.toWei("10000000000000000000000000"));
    await token.connect(accounts[2]).approve(gov.address, h.toWei("10000"));
    await flex
      .connect(accounts[1])
      .depositStake(h.toWei("10000000000000000000000000"));
    let qd = abiCoder.encode(["string", "uint"], ["SpotPrice", 1]);
    let qi = ethers.utils.keccak256(qd);
    await flex.connect(accounts[1]).submitValue(qi, h.bytes(200), 0, qd);
    blocky = await h.getBlock();

    for (i = 0; i < 5; i++) {
      await gov.connect(accounts[2]).beginDispute(qi, blocky.timestamp);
      await gov.connect(accounts[3]).vote(i + 1, false, false);
      // vote 9999 more times
      for (j = 0; j < 9999; j++) {
        await h.expectThrow(gov.connect(accounts[3]).vote(1, false, false));
      }
      await h.advanceTime(86400 * 6);
      await gov.tallyVotes(i + 1);
    }
    ...
    encoded = web3.utils.encodePacked(
      { value: qi, type: "bytes32" },
      { value: blocky.timestamp, type: "uint256" }
    );
    hash = h.hash(encoded);
    voteRounds = await gov.getVoteRounds(hash);
    assert(voteRounds.length == 5, "Should be 5 vote rounds");
  });
  ...
  it("Dispute fee capped at stake amount, many reports", async function () {
    ...
    await flex
      .connect(reporter1)
      .submitValue(ETH_QUERY_ID, h.bytes(101), 0, ETH_QUERY_DATA);
    blocky1 = await h.getBlock();
    await flex
      .connect(reporter2)
      .submitValue(ETH_QUERY_ID, h.bytes(102), 0, ETH_QUERY_DATA);
    blocky2 = await h.getBlock();
    await flex
      .connect(reporter3)
      .submitValue(ETH_QUERY_ID, h.bytes(103), 0, ETH_QUERY_DATA);
    blocky3 = await h.getBlock();
    await flex
      .connect(reporter4)
      .submitValue(ETH_QUERY_ID, h.bytes(104), 0, ETH_QUERY_DATA);
    blocky4 = await h.getBlock();
    await flex
      .connect(reporter5)
      .submitValue(ETH_QUERY_ID, h.bytes(105), 0, ETH_QUERY_DATA);
    blocky5 = await h.getBlock();
    await flex
      .connect(reporter6)
      .submitValue(ETH_QUERY_ID, h.bytes(106), 0, ETH_QUERY_DATA);
  ...
  it("Test 1000 open disputes on a queryId", async function () {
    ...
    reporter6 = accounts[14];
    disputer = accounts[5];
    const REPORTER_STAKE_AMOUNT = h.toWei("10000");
    await token.mint(reporter1.address, REPORTER_STAKE_AMOUNT);
    await token.mint(reporter2.address, REPORTER_STAKE_AMOUNT);
    await token.mint(reporter3.address, REPORTER_STAKE_AMOUNT);
    await token.mint(reporter4.address, REPORTER_STAKE_AMOUNT);
    await token.mint(reporter5.address, REPORTER_STAKE_AMOUNT);
    await token.mint(reporter6.address, REPORTER_STAKE_AMOUNT);
    await token.mint(disputer.address, h.toWei("10000"));

    await token.connect(reporter1).approve(flex.address, REPORTER_STAKE_AMOUNT);
    await token.connect(reporter2).approve(flex.address, REPORTER_STAKE_AMOUNT);
    await token.connect(reporter3).approve(flex.address, REPORTER_STAKE_AMOUNT);
    await token.connect(reporter4).approve(flex.address, REPORTER_STAKE_AMOUNT);
    await token.connect(reporter5).approve(flex.address, REPORTER_STAKE_AMOUNT);
    await token.connect(reporter6).approve(flex.address, REPORTER_STAKE_AMOUNT);
    await token.connect(disputer).approve(gov.address, h.toWei("10000"));

    await flex.connect(reporter1).depositStake(REPORTER_STAKE_AMOUNT);
    await flex.connect(reporter2).depositStake(REPORTER_STAKE_AMOUNT);
    await flex.connect(reporter3).depositStake(REPORTER_STAKE_AMOUNT);
    await flex.connect(reporter4).depositStake(REPORTER_STAKE_AMOUNT);
    await flex.connect(reporter5).depositStake(REPORTER_STAKE_AMOUNT);
    await flex.connect(reporter6).depositStake(REPORTER_STAKE_AMOUNT);
    ...
    openDisputes = await gov.getOpenDisputesOnId(ETH_QUERY_ID);
    assert(openDisputes == 1000, "Should be 1000 open disputes");
  });
});
